...
bool CreateResTextureFromWICFileW( const wchar_t* filename, asdx::ResTexture& resTexture )
{
    ...
    resTexture.pPixels      = pPixels;
    resTexture.PixelSize    = imageSize;
   ...
   // マジックを読み込む.
    ...
    for ( size_t j=0; j<resTexture.MipMapCount; ++j )
    {
        ...
        for( size_t i=0; i<resTexture.SurfaceCount; ++i )
        {
            ...
            resTexture.pResources[ idx ].pPixels    = pPixelData + offset;
    ...
    // ピクセルデータ設定.
    resTexture.pPixels = pPixelData;
    resTexture.PixelSize = pixelSize;
